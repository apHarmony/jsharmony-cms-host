...
function jsHarmonyQueue(platform, config){
  ...
  */
  ...
  this.jsharmonyLogin = function (onSuccess, retries) {
    ...
    async.waterfall([
      ...
      function (cb) {
        ...
        console.log('Please enter the server daemon login username:');
  ...
  this.getQueue = function (queueid) {
    async.waterfall([
      function (cb) {
        platform.log('Requesting next item in queue...');
        wc.reqjson(config.jsHarmonyURL + '/_queue/' + queueid, 'GET', {}, _this.getHTTPHeader(), null, function (err, rslt, rsltjson, xres) {
          ...
          if (err) {
            ...
            var deployment_id = parseInt(rsltjson.deployment_id);
            ...
            if(_this.onMessage) _this.onMessage(msg);
          }
          else {
